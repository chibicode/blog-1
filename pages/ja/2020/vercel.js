...
export default withViews(({ views }) => (
  <Post>
    ...
    <P>
      まずは最近の大きな流れとして、クラウドインフラ、Kubernetes、様々なAPIを提供するサービス等の台頭により、バックエンド開発にコモディティ化の波が押し寄せました。その結果、やりがいがある革新の機会がまだどれだけ残っているか？という点において、フロントエンド開発がより魅力的になりつつあります。そしてフロントエンド開発こそが、多くの企業において価値の源泉であると言えるでしょう。集客や収益化に繋がるコンテンツやプロダクトに、フロントエンドは直結しているからです。
    </P>
    <P>
    ...
    <P>
      ...
      開発を高速化するツールによってJavaScriptエンジニアが恩恵を受けたり、最新のフロントエンド技術によってサービス自体が高速化されることでユーザーが恩恵を受けています。このように近代化が進む中で、さらに良質なフロントエンドの開発者体験を追求するためには、これから述べる3大トレンドに留意する必要があるでしょう。
      もちろん、Vercelが提供するフロントエンド開発者向けのクラウドプラットフォームは、これらのトレンドを念頭に置いて設計されています。
    </P>
    ...
    <P>
      私達の読みは的中しました。現在、フロントエンド開発現場の多くはコンポーネント化への移行を急ピッチで進めています。多くはReactを選び、フロントエンドをバックエンドから分離し、設計を抜本的に見直そうとしているのです。Vercelは、そういった課題に直面しているフロントエンド開発者にとって、必要なものがすべて備わっているプラットフォームでありたいと考えています。
    </P>
    <P>
      Vercelは
      <a href="https://nextjs.org" target="_blank">
    ...
    <P>
      たかがURLだと侮るなかれ。プレビューURLには、複雑な仕組みからフロントエンド開発者を解き放ち、開発スピードを飛躍的に高める効果があるのです。
    </P>
    <P>
    ...
    <P>
      ...
      を用いたセキュアな環境を利用することができるのです。常時SSL化はプレビューURLを誰かと共有する際に役立つだけでなく、本番さながらの環境でテストを行うのにも重宝するでしょう。テスト時にSSL化を徹底していないと、いざ本番にデプロイした際にセキュリティ警告が表示されたり、ユーザー体験が損なわれる可能性が生まれます。ちなみに最近のブラウザでは、
      <a
    ...
    <P>
      ...
      が使われています)。フロントエンドのパフォーマンスにまつわる全ての指標に向けた最適化が行われ、その効果はプレビューURLで確かめることができるのです。
    </P>
    <P>
    ...
    <P>
      ...
      してみてください。オープンなプラットフォームが生んだVercelの、
      <a href="https://nextjs.org" target="_blank">
        どこまでもオープン
      </a>
      な物語が垣間見えると思います。
